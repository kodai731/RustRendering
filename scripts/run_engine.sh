...
On the first full run the file is generated automatically:
  ...
  THYLLORE_INGEST_TOKEN        cannot be derived (write-only secret) — fill in
                               the value you exported when running worker/deploy.sh
EOF
}
...
resolve_test_worker_endpoint() {
    ...
    worker_name="$(sed -n 's/^name = "\(.*\)"$/\1/p' "$REPO_ROOT/worker/wrangler.toml")"
...
generate_full_env_file() {
    ...
    for candidate in "$REPO_ROOT/secrets/public_key.b64" "$REPO_ROOT/worker/secrets/public_key.b64"; do
        if [[ -r "$candidate" ]]; then
...
# THYLLORE_INGEST_TOKEN is the value exported when deploying worker/deploy.sh;
