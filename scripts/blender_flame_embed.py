...
def add_campfire():
    ...
    if existing:
        print(f"[blender_flame_embed] scene already holds flames {existing}", flush=True)
        return
    if bpy.context.mode != "OBJECT":
        bpy.ops.object.mode_set(mode="OBJECT")
    bpy.ops.thyllore.flame_add()
    print(f"[blender_flame_embed] added {bpy.context.active_object.name}", flush=True)
...
def take_screenshot():
    ...
    if found is None:
        print("[blender_flame_embed] no VIEW_3D area for screenshot", flush=True)
    else:
        window, area, region = found
        with bpy.context.temp_override(window=window, area=area, region=region):
            bpy.ops.screen.screenshot_area(filepath=path)
        print(f"[blender_flame_embed] screenshot -> {path}", flush=True)
