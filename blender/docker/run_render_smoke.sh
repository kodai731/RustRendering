...
docker run --rm -v "$REPO_ROOT:$REPO_ROOT" -w "$REPO_ROOT" thyllore-blender-xvfb:local sh -c "xvfb-run -a -s '-screen 0 1280x720x24' blender -noaudio --python-exit-code 1 --python '$REPO_ROOT/blender_flame_addon/tests/render_smoke.py' > '$LOG' 2>&1" || true

grep -v '^\s*|' "$LOG" | grep -v gpu.debug | tail -20

grep -q "^ADDON_SMOKE ok" "$LOG" && grep -q "^RENDER_SMOKE ok" "$LOG"
