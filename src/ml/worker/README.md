...
(`${DocumentPath}/Rust_Rendering/Design/20260719_curve_copilot_private_mode_offline/`).
...
Both routes require `Authorization: Bearer <INGEST_TOKEN>`.
...
truth) and uploads the worker, its R2 binding, plain-text vars, and the
...
mode B builds via `THYLLORE_FULL_TOKEN_PUBKEY_B64`.
...
The script (shared helpers in `lib_e2e.sh`) generates an Ed25519 keypair,
builds the wheel with its public key baked in, starts local workerd, and
asserts the full chain:

```
local workerd (real index.mjs)  --Ed25519 full_token-->  wheel -> ctx64
```
