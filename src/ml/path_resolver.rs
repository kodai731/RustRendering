pub fn resolve_curve_copilot_model_path() -> Option<String> {
    if let Some(path) = thyllore_ml_core::model_path::resolve_v1_curve_copilot_model_path() {
        log!("Using v1 curve_copilot model: {}", path);
        return Some(path);
    }

    log_warn!(
        "v1 curve_copilot model not found. For development, set {} to your SharedData \
        ...
        crate::paths::V1_CURVE_COPILOT_FILENAME,
