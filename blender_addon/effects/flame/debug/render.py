...
def capture_scene_depth(scene, w, h, near):
    ...
    r_channel = buf[0::4]
    depth_values = [coordinates.z_pass_to_engine_depth(z, near) for z in r_channel]
    return depth_values


def depth_texture(depth_values, w, h):
    import gpu
...
def save_exr(scene, exr_path, pixels, width, height):
    import bpy

    settings = scene.render.image_settings
    saved = (settings.file_format, settings.color_mode, settings.color_depth, settings.exr_codec)
    img = bpy.data.images.new("temp", width=width, height=height, float_buffer=True)
    ...
    try:
        ...
        settings.file_format, settings.color_mode, settings.color_depth, settings.exr_codec = saved
        bpy.data.images.remove(img)


def blender_camera_to_engine(cam_matrix_world, angle_y, aspect, clip_start):
    engine_world = coordinates.blender_camera_to_engine_matrix(cam_matrix_world)
    view = coordinates.engine_view_matrix(engine_world)
    proj = coordinates.engine_projection(angle_y, aspect, clip_start)
...
def render_flame_sequence(scene, obj, out_dir, frame_start, frame_end, write_npy=False, use_scene_depth=False):
    ...
    for frame in range(frame_start, frame_end + 1):
        ...
        for o in scene.objects:
            if o.type == "LIGHT":
                light_pos = coordinates.blender_to_engine_point(o.matrix_world.translation)
                break
        ...
        depth_tex = None if depth_values is None else depth_texture(depth_values, rx, ry)
        tex = renderer.render(view, proj, camera_pos, light_pos, params, time, position, rotation, rx, ry, depth_tex=depth_tex)
        rows = tex.read().to_list()
        pixels = [px for row in rows for px in row]

        exr_path = sequence_path(out_dir, obj.name, frame)
        save_exr(scene, exr_path, pixels, rx, ry)
