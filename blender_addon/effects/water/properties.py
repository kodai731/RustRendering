...
def water_render_params(props) -> dict:
    ...
    preset_values = fx.water_preset_params(props.preset)
...
def build_water_property_group():
    ...
    ui_params = select_exposed_params(fx.water_ui_params())
    param_names = [p["name"] for p in ui_params]

    def apply_preset(self, context):
        preset_values = fx.water_preset_params(self.preset)
    ...
    return type("ThylloreWaterProperties", (bpy.types.PropertyGroup,), attrs)
